...
const PdfGenerator = ({ order }) => {
    ...
    const generatePDF = () => {
        ...
        doc.text(`Handling & Freight charges: ${order.freightCharges || 'Ex – Bhiwandi'}`, startX + columnWidth + 4, deliveryStartY + cellHeight + 6);
        
        doc.rect(startX, deliveryStartY + cellHeight * 2, mentionRowWidth, cellHeight);
        doc.text(`Transport Name : - ${order.transportName || 'Bhosale Tempo Service Shirwal Branch Godown delivery'}`, startX + 4, deliveryStartY + cellHeight * 2 + 6);
